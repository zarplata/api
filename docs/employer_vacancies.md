...
professional_roles[].id | string | индификатор профессиональной роли [из справочника](suggests.md#proff-roles)
...
<a name="allow_messages"></a> allow_messages | boolean или null | возможность [переписки с кандидатами](https://inboxemp.zarplata.ru/) по данной вакансии
...
При указании параметра `with_professional_roles=true` поле `specializations` не возвращается в ответе, а поле [professional_roles](professional_roles.md) появляется вместе с другими полями
