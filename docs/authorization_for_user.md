...
После получения `authorization_code` приложению необходимо сделать сервер-сервер запрос `POST https://hr.zarplata.ru/oauth/token` для обмена полученного
...
```
DELETE https://api.zarplata.ru/oauth/token
```
...
Для получения списка рабочих аккаунтов менеджера и для работы под разными рабочими аккаунтами менеджера необходимо прочитать документацию по [рабочим аккаунтам менеджера](https://api.zarplata.ru/openapi/redoc#tag/Menedzhery-rabotodatelya/paths/~1manager_accounts~1mine/get)
