...
* Базовый URL — `https://api.zarplata.ru/`
...
В запросе необходимо передавать заголовок `User-Agent` Если не отправлен этот заголовок, то ответом будет `400 Bad Request`. 
