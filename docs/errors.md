...
default_vacancy_branded_template_is_invalid_or_not_enough_purchased_services | в запросе указан шаблон, который отсутствует в списке доступных шаблонов (этот список можно получить [запросом](https://github.com/zarplata/api/blob/master/docs/employer_vacancy_branded_templates.md#list) ). Также шаблон может отсутствовать в списке доступных шаблонов, если не оплачена услуга использования [брендированного шаблона вакансии](https://hr.zarplata.ru/price?from=menu#branding) 
...
Элементы массива аналогичны [результату, выдаваемому в списке рабочих аккаунтов](https://api.zarplata.ru/openapi/redoc#tag/Menedzhery-rabotodatelya/paths/~1manager_accounts~1mine/get)
